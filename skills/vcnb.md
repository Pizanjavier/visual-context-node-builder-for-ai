...
> **Portable skill for Claude Code.** Copy this file to `.claude/commands/` in any project
> where the [Visual Context Node Builder](https://marketplace.visualstudio.com/items?itemName=pizan.visual-context-node-builder) extension is installed.
>
> **Usage:** `/vcnb <description or command>`
>
> Examples:
...
You are creating a recipe for the **Visual Context Node Builder** VS Code extension.
...
- `filePath`: **absolute path** in the user's workspace. Use the full path based on the
  current working directory (discover it with `pwd` if needed).
...
understanding a diff, or assessing blast radius, you may want to recommend this instead:
